...
readpw(Display *dpy, struct xrandr *rr, struct lock **locks, int nscreens,
       const char *hash, int oneButton, const char* lock_message)
{
		...
		if (ev.type == KeyPress) {
			...
			if (running) {
				for (screen = 0; screen < nscreens; screen++) {
					...
					writemessage(dpy, locks[screen]->win, screen, lock_message);
				}
			}
...
static int lock(const char* lock_message, int oneButton, int runCommand, char*const* argv) {
	...
	for (nlocks = 0, s = 0; s < nscreens; s++) {
		if ((locks[s] = lockscreen(dpy, &rr, s)) != NULL) {
			writemessage(dpy, locks[s]->win, s, lock_message);
			nlocks++;
		} else {
			break;
		}
	...
	readpw(dpy, &rr, locks, nscreens, hash, oneButton, lock_message);
